...
async Task<KeyMap?> GetSpectrumKeyMapAsync()
{
...
async Task<TOut> SetAndGetFeatureAsync<TIn, TOut>(SafeFileHandle handle, TIn input)
    where TIn : notnull
    where TOut : struct
{
...
try
{
    var keyMap = await GetSpectrumKeyMapAsync().ConfigureAwait(false);
