...
public partial class SettingsPage
{
    ...
    private void HighPerformanceUIToggle_Click(object sender, RoutedEventArgs e)
    {
        ...
        SnackbarHelper.Show(
            "High Performance UI",
            state.Value 
                ? "Hardware acceleration enabled. Restart the app for changes to take effect."
                : "Battery-saving mode enabled. Restart the app for changes to take effect.",
            SnackbarType.Success);
    }
    ...
    private void KeyboardBacklightTimeoutSlider_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
    {
        ...
        int timeout = (int)e.NewValue;
        _settings.Store.KeyboardBacklightTimeoutSeconds = timeout;
        _settings.SynchronizeStore();
        
        _keyboardBacklightTimeoutText.Text = timeout == 0 ? "0 (Off)" : $"{timeout}s";
        
        // Start or stop the service based on timeout value
        try
        {
            var service = IoCContainer.Resolve<KeyboardBacklightTimeoutService>();
            if (timeout > 0)
                service.Start();
            else
                service.Stop();
        ...
        {
            Log.Instance.Trace($"[KeyboardBacklightTimeout] Service start/stop failed: {ex.Message}");
        }
    }
    ...
    private async void ExportSettings_Click(object sender, RoutedEventArgs e)
    {
        ...
        var dialog = new SaveFileDialog
        {
            ...
            FileName = $"LLT_Settings_{DateTime.Now:yyyyMMdd}.json"
        };
        ...
        try
        {
            var syncService = IoCContainer.Resolve<ProfileSyncService>();
            var success = await syncService.ExportAsync(dialog.FileName);

            if (success)
                SnackbarHelper.Show("Export", "Settings exported successfully!", SnackbarType.Success);
            else
                SnackbarHelper.Show("Export", "Failed to export settings", SnackbarType.Error);
        }
        catch (Exception ex)
        {
            Log.Instance.Trace($"Export failed: {ex.Message}");
            SnackbarHelper.Show("Export", $"Error: {ex.Message}", SnackbarType.Error);
        }
    }
    ...
    private async void ImportSettings_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            var syncService = IoCContainer.Resolve<ProfileSyncService>();
            var success = await syncService.ImportAsync(dialog.FileName);

            if (success)
            {
                SnackbarHelper.Show("Import", "Settings imported! Restart recommended.", SnackbarType.Success);
            }
            else
            {
                SnackbarHelper.Show("Import", "Failed to import settings", SnackbarType.Error);
            }
        }
        catch (Exception ex)
        {
            Log.Instance.Trace($"Import failed: {ex.Message}");
            SnackbarHelper.Show("Import", $"Error: {ex.Message}", SnackbarType.Error);
        }
    }
