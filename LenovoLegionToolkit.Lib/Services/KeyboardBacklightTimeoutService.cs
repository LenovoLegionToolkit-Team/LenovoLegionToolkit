...
public class KeyboardBacklightTimeoutService : IDisposable
{
    ...
    public void Start()
    {
        if (_settings.Store.KeyboardBacklightTimeoutSeconds <= 0)
            return;
            
        _lastInputTime = GetIdleTime();
        _checkTimer.Start();
        Log.Instance.Trace($"[BacklightTimeout] Started with {_settings.Store.KeyboardBacklightTimeoutSeconds}s timeout");
    }
    ...
    private async void CheckTimer_Elapsed(object? sender, ElapsedEventArgs e)
    {
        try
        {
            int timeoutSeconds = _settings.Store.KeyboardBacklightTimeoutSeconds;
            if (timeoutSeconds <= 0)
            {
                RestoreBacklight();
                return;
